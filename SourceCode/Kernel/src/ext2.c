    ...
    DirectoryEntry* vfsDirectoryEntry, char* name)
{
    if ((ext2IndexNode->typeAndPermissions & 0xF000) == EXT2_INDEX_NODE_TYPE_DIRECTORY) {
        ...
        DirectoryEntry* directoryEntry = ext2FileSystem->superblock.operations.createDirectoryEntry(&ext2FileSystem->superblock, name);
        IndexNode* indexNode = ext2FileSystem->superblock.operations.createIndexNode(&ext2FileSystem->superblock, directoryEntry);
        ...
        Ext2DirectoryEntry* dEntry = (Ext2DirectoryEntry*)((uint32_t)ext2FileSystem->data + ext2IndexNode->directBlockPointer0 * ext2FileSystem->blockSize);
        ...
        while (*(uint32_t*)((uint32_t)dEntry) != 0) {
            ...
            Ext2IndexNode* nextNode = (Ext2IndexNode*)((uint32_t)ext2FileSystem->blockGroups[blockGroup].indexNode + (((uint32_t)dEntry->indexNode - 1) % ext2FileSystem->ext2SuperBlock->eachBlockGroupIndexNodeNums) * EXT2_INDEX_NODE_STRUCTURE_SIZE);
            char name[0xFF] = { '\0' };
...
KernelStatus ext2_fs_default_mount(Ext2FileSystem* ext2FileSystem, char* mountName, void* data)
{
    ...
    // make root
    DirectoryEntry* rootDirectoryEntry = ext2FileSystem->superblock.operations.createDirectoryEntry(&ext2FileSystem->superblock, mountName);
    IndexNode* rootIndexNode = ext2FileSystem->superblock.operations.createIndexNode(&ext2FileSystem->superblock, rootDirectoryEntry);
    ...
    Ext2BlockGroup* blockGroup = (Ext2BlockGroup*)kernelHeap.operations.alloc(&kernelHeap, blockGroupNums * sizeof(Ext2BlockGroup));
    ...
    for (uint32_t blockGroupDescriptorIndex = 0; blockGroupDescriptorIndex < blockGroupNums;
        ...
        blockGroup[blockGroupDescriptorIndex].blockGroupDescriptor = (Ext2BlockGroupDescriptor*)((uint32_t)blockGroup[blockGroupDescriptorIndex].superBlock + blockSize + blockGroupDescriptorIndex * EXT2_BLOCK_GROUP_DESCRIPTOR_SIZE);

        blockGroup[blockGroupDescriptorIndex].dataBlockBitmap = (Ext2DataBlockBitmap*)((uint32_t)data + blockGroup[blockGroupDescriptorIndex].blockGroupDescriptor->blockUsageBitMapBlock * blockSize);

        blockGroup[blockGroupDescriptorIndex].indexNodeBitmap = (Ext2IndexNodeBlockBitmap*)((uint32_t)data + blockGroup[blockGroupDescriptorIndex].blockGroupDescriptor->indexNodeUsageBitMapBlock * blockSize);

        blockGroup[blockGroupDescriptorIndex].indexNode = (Ext2IndexNode*)((uint32_t)data + blockGroup[blockGroupDescriptorIndex].blockGroupDescriptor->indexNodeTableBlockBlock * blockSize);
    ...
    Ext2DirectoryEntry* ext2RootDirectoryEntry = (Ext2DirectoryEntry*)((uint32_t)data + root->directBlockPointer0 * blockSize);
...
uint32_t ext2_get_data_block(Ext2FileSystem* ext2FileSystem, Ext2IndexNode* ext2IndexNode, uint32_t blockIndex)
{
    ...
    uint32_t triplyIndirectBlocksMax = directBlockMax + blockPointerNumsInEachBlock * blockPointerNumsInEachBlock * blockPointerNumsInEachBlock;
    ...
    default:
        if ((blockIndex > directBlockMax - 1) && (blockIndex < singlyIndirectBlocksMax)) {
            uint32_t singlyIndirectBlock = (uint32_t)(ext2FileSystem->data + ext2IndexNode->singlyIndirectBlockPointer * ext2FileSystem->blockSize);
            ...
            uint32_t* level1DataBlock = (uint32_t*)(ext2FileSystem->data + ext2IndexNode->doublyIndirectBlockPointer * ext2FileSystem->blockSize);
    ...
    uint32_t blockIndex)
{
    ...
    for (uint32_t i = 0; i < ext2FileSystem->blockSize; i++) {
        buf[blockIndex * ext2FileSystem->blockSize + i] = ((char*)(ext2FileSystem->data + dataBlock * ext2FileSystem->blockSize))[i];
    }
}
...
uint32_t ext2_fs_default_read(Ext2FileSystem* ext2FileSystem, Ext2IndexNode* ext2IndexNode, char* buf, uint32_t count)
{
    ...
    if (blockNeed == 0) {
        ...
        for (uint32_t i = 0; i < restBytes; i++) {
            buf[blockNeed * ext2FileSystem->blockSize + i] = ((char*)(ext2FileSystem->data + ext2_get_data_block(ext2FileSystem, ext2IndexNode, blockNeed) * ext2FileSystem->blockSize))[i];
        }
        return count;
    }
}
