        ...
        if (window->isWindowNeedUpdate) {
            if (window->component.colorMode == RGB) {
                ...
                gfx2d_fill_rect(context, window->component.position.x, window->component.position.y,
                    ...
                    window->header.foreground.a << 24 | window->component.background.r << 16 | window->component.background.g << 8 | window->component.background.b);
            }
            ...
            gfx2d_fill_rect(context, window->component.position.x, window->component.position.y,
                ...
                window->header.foreground.a << 24 | window->header.background.r << 16 | window->header.background.g << 8 | window->header.background.b);
                ...
                for (uint32_t j = 0; j < 16; j++) {
                    if ((bitmap[i] & (0x1 << j)) > 0) {
                        gfx2d_write_pixel_color(context, window->component.position.x + j + DEFAULT_PADDING,
                            ...
                            window->header.foreground.r << 16 | window->header.foreground.g << 8 | window->header.foreground.b);
                    }
                }
            }
        ...
        if (window->component.boxShadow.enable && window->isShadowNeedUpdate) {
            ...
            for (uint32_t i = 1; i < window->component.boxShadow.width; i++) {
                ...
                gfx2d_fill_rect(context, window->component.position.x - i, window->component.position.y - i,
                    ...
                    window->component.boxShadow.color.r << 16 | window->component.boxShadow.color.g << 8 | window->component.boxShadow.color.b | alpha << 24);
            }
            ...
            for (uint32_t i = 0; i < window->component.boxShadow.width; i++) {
                ...
                gfx2d_fill_rect(context, window->component.position.x - i,
                    ...
                    window->component.position.y + window->component.size.height + DEFAULT_WINDOW_HEADER_HEIGHT + i + 1,
                    window->component.boxShadow.color.r << 16 | window->component.boxShadow.color.g << 8 | window->component.boxShadow.color.b | alpha << 24);
            }
    ...
    if (children != nullptr) {
        for (uint32_t i = 0; i < children->index; i++) {
            ...
            case CANVAS: {
                ...
                canvas->component.position.y = window->component.position.y + DEFAULT_WINDOW_HEADER_HEIGHT + window->component.padding.top;
            ...
            case VIEW3D: {
                ...
                view->component.position.y = window->component.position.y + DEFAULT_WINDOW_HEADER_HEIGHT + window->component.padding.top;
            ...
            case CONTAINER: {
                ...
                innerContainer->component.position.y = innerContainer->component.position.y + window->component.position.y + DEFAULT_WINDOW_HEADER_HEIGHT + window->component.padding.top;
