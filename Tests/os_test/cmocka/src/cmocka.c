...
static size_t check_for_leftover_values(const ListNode *const map_head, const char *const error_message,
...
static void free_symbol_map_value(const void *value, void *cleanup_value_data) {
  ...
  assert_non_null(value);
  if (children == 0) {
    list_free(&map_value->symbol_values_list_head, free_value, NULL);
  } else {
    list_free(&map_value->symbol_values_list_head, free_symbol_map_value, (void *)((uintptr_t)children - 1));
  }
...
static void add_symbol_value(ListNode *const symbol_map_head, const char *const symbol_names[],
  ...
  if (number_of_symbol_names == 1) {
    ...
    add_symbol_value(&target_map_value->symbol_values_list_head, &symbol_names[1], number_of_symbol_names - 1, value,
                     refcount);
  }
}
...
void _function_called(const char *const function, const char *const file, const int line) {
  ...
  if (rc) {
    ...
    if (value_node->refcount < -1) {
      ...
      if (cmp != 0) {
        ...
        if (expected_call == NULL || value_node == first_value_node->prev) {
          cm_print_error(SOURCE_LOCATION_FORMAT ": error: No expected mock calls matching "
    ...
    if (cmp == 0) {
      ...
      cm_print_error(SOURCE_LOCATION_FORMAT ": error: Expected call to %s but received called() "
                                            "in %s\n",
                     file, line, expected_call->function, function);
      exit_test(1);
    }
  } else {
    cm_print_error(SOURCE_LOCATION_FORMAT ": error: No mock calls expected but called() was "
                                          "invoked in %s\n",
                   file, line, function);
    exit_test(1);
  }
...
static int double_compare(const double left, const double right, const double epsilon) {
  double absLeft;
  double absRight;
  double largest;
  double relDiff;
 ...
 */
static int double_values_equal_display_error(const double left, const double right, const double epsilon) {
  const int equal = double_compare(left, right, epsilon);

  if (!equal) {
    cm_print_error(DoublePrintfFormat " != " DoublePrintfFormat "\n", left, right);
  }
 ...
 * equal an error is displayed and 0 is returned.
 */
static int double_values_not_equal_display_error(const double left, const double right, const double epsilon) {
 ...
 * displayed.
 */
static int value_in_set_display_error(const LargestIntegralType value, const CheckIntegerSet *const check_integer_set,
                                      const int invert) {
...
static int integer_not_in_range_display_error(const LargestIntegralType value, const LargestIntegralType range_min,
  ...
  cm_print_error(LargestIntegralTypePrintfFormatDecimal " is within the range " LargestIntegralTypePrintfFormatDecimal
                                                        "-" LargestIntegralTypePrintfFormatDecimal "\n",
                 value, range_min, range_max);
  return 0;
}
...
static int check_in_set(const LargestIntegralType value, const LargestIntegralType check_value_data) {
  return value_in_set_display_error(value, cast_largest_integral_type_to_pointer(CheckIntegerSet *, check_value_data),
                                    0);
}
...
static void expect_set(const char *const function, const char *const parameter, const char *const file, const int line,
                       const LargestIntegralType values[], const size_t number_of_values,
                       const CheckParameterValue check_function, const int count) {
  CheckIntegerSet *const check_integer_set =
      (CheckIntegerSet *)malloc(sizeof(*check_integer_set) + (sizeof(values[0]) * number_of_values));
 ...
 * register a check event. */
static void expect_range(const char *const function, const char *const parameter, const char *const file,
...
static int check_memory(const LargestIntegralType value, const LargestIntegralType check_value_data) {
  ...
  return memory_equal_display_error(cast_largest_integral_type_to_pointer(const char *, value),
                                    (const char *)check->memory, check->size);
}

/* Create the callback data for check_memory() or check_not_memory() and
 * register a check event. */
static void expect_memory_setup(const char *const function, const char *const parameter, const char *const file,
                                const int line, const void *const memory, const size_t size,
                                const CheckParameterValue check_function, const int count) {
...
static int check_not_memory(const LargestIntegralType value, const LargestIntegralType check_value_data) {
  CheckMemoryData *const check = cast_largest_integral_type_to_pointer(CheckMemoryData *, check_value_data);
  assert_non_null(check);
  return memory_not_equal_display_error(cast_largest_integral_type_to_pointer(const char *, value),
                                        (const char *)check->memory, check->size);
}
  ...
  if (rc) {
    ...
    if (!check_succeeded) {
      cm_print_error(SOURCE_LOCATION_FORMAT
                     ...
                     file, line, parameter_name, function_name, global_last_parameter_location.file,
...
void _assert_float_not_equal(const float a, const float b, const float epsilon, const char *const file,
                             const int line) {
...
void _assert_double_equal(const double a, const double b, const double epsilon, const char *const file,
                          const int line) {
...
void _assert_double_not_equal(const double a, const double b, const double epsilon, const char *const file,
                              const int line) {
...
void *_test_malloc(const size_t size, const char *file, const int line) {
  ...
  ptr = (char *)(((size_t)block + MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData) + MALLOC_ALIGNMENT) &
                 ~(MALLOC_ALIGNMENT - 1));
      ...
      for (j = 0; j < MALLOC_GUARD_SIZE; j++) {
        ...
        if (diff) {
          cm_print_error(SOURCE_LOCATION_FORMAT
                         ...
                         file, line, ptr, (unsigned long)block_info.data->size, block_info.data->location.file,
                         block_info.data->location.line, (void *)&guard[j]);
          _fail(file, line);
        }
      }
    }
...
static const ListNode *check_point_allocated_blocks(void) { return get_allocated_blocks_list()->prev; }
...
static size_t display_allocated_blocks(const ListNode *const check_point) {
  ...
  for (node = check_point->next; node != head; node = node->next) {
    ...
    cm_print_error(SOURCE_LOCATION_FORMAT ": note: block %p allocated here\n", block_info.data->location.file,
                   block_info.data->location.line, block_info.data->block);
    allocated_blocks++;
  }
...
static void cmprintf_group_finish_xml(const char *group_name, size_t total_executed, size_t total_failed,
...
static void cmprintf_group_start_standard(const char *group_name, const size_t num_tests) {
  print_message("[==========] %s: Running %zu test(s).\n", group_name, num_tests);
}

static void cmprintf_group_finish_standard(const char *group_name, size_t total_executed, size_t total_passed,
                                           size_t total_failed, size_t total_errors, size_t total_skipped,
                                           struct CMUnitTestState *cm_tests) {
  size_t i;

  print_message("[==========] %s: %zu test(s) run.\n", group_name, total_executed);
  print_error("[  PASSED  ] %u test(s).\n", (unsigned)(total_passed));

  if (total_skipped) {
    print_error("[  SKIPPED ] %s: %zu test(s), listed below:\n", group_name, total_skipped);
    for (i = 0; i < total_executed; i++) {
      struct CMUnitTestState *cmtest = &cm_tests[i];
...
static void cmprintf_group_start_tap(const size_t num_tests) { print_message("1..%u\n", (unsigned)num_tests); }

static void cmprintf_group_finish_tap(const char *group_name, size_t total_executed, size_t total_passed,
...
static void cmprintf_tap(enum cm_printf_type type, size_t test_number, const char *test_name,
...
static void cmprintf_group_start(const char *group_name, const size_t num_tests) {
...
static void cmprintf_group_finish(const char *group_name, size_t total_executed, size_t total_passed,
                                  size_t total_failed, size_t total_errors, size_t total_skipped, double total_runtime,
                                  struct CMUnitTestState *cm_tests) {
  enum cm_message_output output;
  ...
  switch (output) {
  case CM_OUTPUT_STDOUT:
    cmprintf_group_finish_standard(group_name, total_executed, total_passed, total_failed, total_errors, total_skipped,
                                   cm_tests);
    break;
  case CM_OUTPUT_SUBUNIT:
    break;
  case CM_OUTPUT_TAP:
    ...
    break;
  case CM_OUTPUT_XML:
    cmprintf_group_finish_xml(group_name, total_executed, total_failed, total_errors, total_skipped, total_runtime,
                              cm_tests);
    break;
  }
}
...
void cmocka_set_message_output(enum cm_message_output output) { global_msg_output = output; }

void cmocka_set_test_filter(const char *pattern) { global_test_filter_pattern = pattern; }

void cmocka_set_skip_filter(const char *pattern) { global_skip_filter_pattern = pattern; }
 ...
 * CMOCKA TEST RUNNER
 ****************************************************************************/
static int cmocka_run_one_test_or_fixture(const char *function_name, CMUnitTestFunction test_func,
                                          CMFixtureFunction setup_func, CMFixtureFunction teardown_func,
                                          void **const volatile state, const void *const heap_check_point) {
  const ListNode *const volatile check_point =
      (const ListNode *)(heap_check_point != NULL ? heap_check_point : check_point_allocated_blocks());
...
static int cmocka_run_group_fixture(const char *function_name, CMFixtureFunction setup_func,
                                    CMFixtureFunction teardown_func, void **state, const void *const heap_check_point) {
  int rc;
...
static int cmocka_run_one_tests(struct CMUnitTestState *test_state) {
  ...
  if (test_state->test->setup_func != NULL) {
    ...
    rc = cmocka_run_one_test_or_fixture(test_state->test->name, NULL, test_state->test->setup_func, NULL,
  ...
  if (rc == 0) {
    rc = cmocka_run_one_test_or_fixture(test_state->test->name, test_state->test->test_func, NULL, NULL,
  ...
  if (rc == 0 && test_state->test->teardown_func != NULL) {
    rc = cmocka_run_one_test_or_fixture(test_state->test->name, NULL, NULL, test_state->test->teardown_func,
                                        &test_state->state, test_state->check_point);
    if (rc != 0) {
      test_state->status = CM_TEST_ERROR;
...
int _cmocka_run_group_tests(const char *group_name, const struct CMUnitTest *const tests, const size_t num_tests,
  ...
  for (i = 0; i < num_tests; i++) {
    if (tests[i].name != NULL &&
        (tests[i].test_func != NULL || tests[i].setup_func != NULL || tests[i].teardown_func != NULL)) {
  ...
  cmprintf_group_finish(group_name, total_executed, total_passed, total_failed, total_errors, total_skipped,
...
int _run_test(const char *const function_name, const UnitTestFunction Function, void **const volatile state,
  ...
  const ListNode *const volatile check_point =
      (const ListNode *)(heap_check_point ? heap_check_point : check_point_allocated_blocks());
  void *current_state = NULL;
  volatile int rc = 1;
  int handle_exceptions = 1;
#ifdef _WIN32
...
int _run_group_tests(const UnitTest *const tests, const size_t number_of_tests) {
  ...
  if (teardown != NULL) {
    ...
    failed = _run_test(teardown_name, teardown, current_state, UNIT_TEST_FUNCTION_TYPE_GROUP_TEARDOWN,
